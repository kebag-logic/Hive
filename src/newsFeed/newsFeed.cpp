...
private:
	...
	virtual void checkForNews(std::uint64_t const lastCheckTime) noexcept override
	{
		if (!_checkInProgress)
		{
			QNetworkRequest request{ QUrl{ QString{ "%1?fileURL=%2&lastCheckTime=%3&buildNumber=%4&os=%5" }.arg(hive::internals::newsFeedUrl.c_str()).arg(hive::internals::newsFeedFileUrl.c_str()).arg(lastCheckTime).arg(hive::internals::buildNumber).arg(hive::internals::systemName) } };
