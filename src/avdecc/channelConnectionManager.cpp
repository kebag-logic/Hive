...
private:
	...
	*/
	...
	std::pair<la::avdecc::entity::model::StreamIndex, la::avdecc::entity::model::StreamIndex> getRedundantTalkerStreamIndexPair(la::avdecc::UniqueIdentifier const talkerEntityId, la::avdecc::entity::model::StreamIndex const primaryTalkerStreamIndex, la::avdecc::controller::model::VirtualIndex const talkerStreamVirtualIndex, la::avdecc::UniqueIdentifier const listenerEntityId, la::avdecc::entity::model::StreamIndex const listenerStreamIndex) const noexcept
	{
		auto result = std::pair<la::avdecc::entity::model::StreamIndex, la::avdecc::entity::model::StreamIndex>();
		auto const& manager = hive::modelsLibrary::ControllerManager::getInstance();
	...
	std::pair<la::avdecc::entity::model::StreamIndex, la::avdecc::entity::model::StreamIndex> getRedundantListenerStreamIndexPair(la::avdecc::UniqueIdentifier const talkerEntityId, la::avdecc::entity::model::StreamIndex const talkerStreamIndex, la::avdecc::UniqueIdentifier const listenerEntityId, la::avdecc::entity::model::StreamIndex const primaryListenerStreamIndex, la::avdecc::controller::model::VirtualIndex const listenerStreamVirtualIndex) const noexcept
	{
		auto result = std::pair<la::avdecc::entity::model::StreamIndex, la::avdecc::entity::model::StreamIndex>();
		auto const& manager = hive::modelsLibrary::ControllerManager::getInstance();
	...
	*/
	virtual std::shared_ptr<TargetConnectionInformations> getChannelConnections(la::avdecc::UniqueIdentifier const& sourceEntityId, ChannelIdentification const sourceChannelIdentification) const noexcept
	{
		...
		auto configurationNode = la::avdecc::controller::model::ConfigurationNode{ configurationIndex };
		try
		{
		...
		// collect the talker side mappings (static and dynamic)
		...
		// dump all of the talker streams that are have an associated mapping to the given source channel into a listing of stream+channel pairs for easier handling
		auto sourceStreamChannelPairs = std::vector<std::pair<la::avdecc::entity::model::StreamIndex, std::uint16_t>>{};
		for (auto const& mapping : mappings)
		{
			if (mapping.clusterOffset == clusterIndex - baseCluster && clusterChannel == mapping.clusterChannel)
			{
				sourceStreamChannelPairs.push_back(std::make_pair(mapping.streamIndex, mapping.streamChannel));
			}
		}


		auto processedListenerChannels = std::set<std::tuple<la::avdecc::UniqueIdentifier, la::avdecc::entity::model::ClusterIndex, std::uint16_t>>{};

		auto const& streamConnections = getAllStreamOutputConnections(sourceEntityId);

		// Now find out the connected streams.
		// This is done by
		// 1. iteration over the listing of collected talker side stream+channel pairs that relate to the source channel under investigation in this method
		// 2. iteration over the listing of all outgoing stream connections
		// 3. If the stream used in a mapping and the stream in the connections match, get into the details of channel connection processing
		for (auto const& sourceStreamChannelPair : sourceStreamChannelPairs)
		{
			auto& sourceStream = sourceStreamChannelPair.first;
			auto& sourceStreamChannel = sourceStreamChannelPair.second;
			for (auto const& [listenerStream, streamConnectionInfo] : streamConnections)
			{
				if (streamConnectionInfo.talkerStream.streamIndex == sourceStream)
				{
					// The streamConnection relates to the source channel under investigation in this method,
					// so now we get into the details of resolving the resulting channel connections:
					auto targetControlledEntity = manager.getControlledEntity(listenerStream.entityID);
					if (!targetControlledEntity)
					{
						continue;
					}
					if (!targetControlledEntity->getEntity().getEntityCapabilities().test(la::avdecc::entity::EntityCapability::AemSupported) || !targetControlledEntity->hasAnyConfiguration())
					{
						continue;
					}
					...
					{
						for (auto const& streamPortInputKV : listenerAudioUnitKV.second.streamPortInputs)
						{
							auto const& listenerMappings = streamPortInputKV.second.dynamicModel.dynamicAudioMap;
							for (auto const& listenerMapping : listenerMappings)
							{
								auto listenerClusterChannel = std::make_tuple(listenerStream.entityID, listenerMapping.clusterOffset, listenerMapping.clusterChannel);
								...
								try
								{
									...
									if (listenerMapping.streamIndex == listenerStream.streamIndex && listenerMapping.streamChannel == sourceStreamChannel)
									{
										...
										auto primaryListenerStreamIndexIt = primToRedundantListenerStreamsMap.find(listenerMapping.streamIndex);
										if (primaryListenerStreamIndexIt != primToRedundantListenerStreamsMap.end())
										{
											// If the listenerMapping's streamIndex is found in the keys of the map of primary listener streams,
											// we have completed the info on established primary stream connection
											primaryListenerStreamIndex = primaryListenerStreamIndexIt->first;
											primaryTalkerStreamIndex = streamConnectionInfo.talkerStream.streamIndex;
										}
										else
										{
											// If the listenerMapping's streamIndex is not found in the primarys, we try to find it in the reverse redundant->primary map
											auto streamIndex = redundantToPrimListenerStreamMap.find(listenerMapping.streamIndex);
											if (!connectionInformation->sourceVirtualIndex)
											{
												continue;
											}
											else if (streamIndex != redundantToPrimListenerStreamMap.end())
											{
										...
										connectionInformation->targetClusterChannels.push_back(std::make_pair(listenerMapping.clusterOffset, listenerMapping.clusterChannel));
										connectionInformation->targetAudioUnitIndex = listenerAudioUnitKV.first;
										...
										connectionInformation->isSourceRedundant = controlledEntity->getStreamOutputNode(configurationNode.descriptorIndex, sourceStream).isRedundant;
										connectionInformation->isTargetRedundant = targetControlledEntity->getStreamInputNode(targetConfigurationNode.descriptorIndex, listenerMapping.streamIndex).isRedundant;
	...
	*/
	CheckChannelCreationsPossibleResult checkChannelCreationsPossible(la::avdecc::UniqueIdentifier const& talkerEntityId, la::avdecc::UniqueIdentifier const& listenerEntityId, std::vector<std::pair<avdecc::ChannelIdentification, avdecc::ChannelIdentification>> const& talkerToListenerChannelConnections, bool const allowTalkerMappingChanges, bool const allowRemovalOfUnusedAudioMappings, std::uint16_t const channelUsageHint) const noexcept
	{
		...
		if (!controlledTalkerEntity->getEntity().getEntityCapabilities().test(la::avdecc::entity::EntityCapability::AemSupported) || !controlledTalkerEntity->hasAnyConfiguration())
		{
			return {};
		}
	...
	std::vector<StreamChannelInfo> findAllUsableStreamChannelsOnStreamConnection(la::avdecc::UniqueIdentifier const talkerEntityId, la::avdecc::UniqueIdentifier const listenerEntityId, StreamConnection const streamConnection, bool const isStreamAlreadyConnected, la::avdecc::entity::model::ClusterIndex const talkerClusterOffset, std::uint16_t const talkerClusterChannel, la::avdecc::entity::model::ClusterIndex const listenerClusterOffset, uint16_t const listenerClusterChannel, StreamChannelMappings const& newMappingsTalker, StreamChannelMappings const& newMappingsListener) const noexcept
	{
		...
		auto buildStreamChannelInfo = [talkerEntityId, listenerEntityId, streamConnection, isStreamAlreadyConnected, talkerClusterOffset, listenerClusterOffset](std::uint16_t streamChannel, bool reusesTalkerMapping, bool reusesListenerMapping) -> std::optional<StreamChannelInfo>
		{
			...
			try
			{
				...
				auto const& streamOutputDynamicModel = talkerEntity->getStreamOutputNode(talkerEntity->getCurrentConfigurationNode().descriptorIndex, talkerStreamIndex).dynamicModel;
				auto const& streamInputDynamicModel = listenerEntity->getStreamInputNode(listenerEntity->getCurrentConfigurationNode().descriptorIndex, listenerStreamIndex).dynamicModel;
	...
	*/
		...
		la::avdecc::UniqueIdentifier const& talkerEntityId, la::avdecc::entity::model::AudioUnitIndex const talkerAudioUnitIndex, la::avdecc::entity::model::StreamPortIndex const talkerStreamPortIndex, la::avdecc::entity::model::ClusterIndex const talkerClusterIndex, la::avdecc::entity::model::ClusterIndex const talkerBaseCluster, std::uint16_t const talkerClusterChannel, la::avdecc::UniqueIdentifier const& listenerEntityId, la::avdecc::entity::model::AudioUnitIndex const listenerAudioUnitIndex, la::avdecc::entity::model::StreamPortIndex const listenerStreamPortIndex, la::avdecc::entity::model::ClusterIndex const listenerClusterIndex, la::avdecc::entity::model::ClusterIndex const listenerBaseCluster, std::uint16_t const listenerClusterChannel) noexcept
	{
		...
		if (!controlledListenerEntity->getEntity().getEntityCapabilities().test(la::avdecc::entity::EntityCapability::AemSupported) || !controlledListenerEntity->hasAnyConfiguration())
		{
			return ChannelDisconnectResult::Unsupported;
		}
	...
	*/
	...
	StreamConnections getPossibleAudioStreamConnectionsBetweenDevices(la::avdecc::UniqueIdentifier const& talkerEntityId, la::avdecc::UniqueIdentifier const& listenerEntityId) const noexcept
	{
		...
		if (!controlledTalkerEntity->getEntity().getEntityCapabilities().test(la::avdecc::entity::EntityCapability::AemSupported) || !controlledListenerEntity->getEntity().getEntityCapabilities().test(la::avdecc::entity::EntityCapability::AemSupported) || !controlledTalkerEntity->hasAnyConfiguration() || !controlledListenerEntity->hasAnyConfiguration())
		{
			return result;
		}

		auto talkerConfigurationNode = la::avdecc::controller::model::ConfigurationNode{ controlledTalkerEntity->getCurrentConfigurationNode() };
		try
		{
		...
		auto listenerConfigurationNode = la::avdecc::controller::model::ConfigurationNode{ controlledListenerEntity->getCurrentConfigurationNode() };
		try
		{
		...
		for (auto const& streamOutputKV : talkerConfigurationNode.streamOutputs)
		{
			...
			for (auto const& streamInputKV : listenerConfigurationNode.streamInputs)
			{
				...
				auto const& streamInputDymaicModel = streamInputKV.second.dynamicModel;
	...
	*/
	std::pair<std::optional<la::avdecc::entity::model::StreamFormat>, std::optional<la::avdecc::entity::model::StreamFormat>> findCompatibleStreamPairFormat(la::avdecc::UniqueIdentifier const& talkerEntityId, la::avdecc::entity::model::StreamIndex const streamOutputIndex, la::avdecc::UniqueIdentifier const& listenerEntityId, la::avdecc::entity::model::StreamIndex const streamInputIndex, la::avdecc::entity::model::StreamFormatInfo::Type const expectedStreamFormatType, std::uint16_t const channelMinSizeHint = 0) const noexcept
	{
		...
		auto streamOutputNode = la::avdecc::controller::model::StreamOutputNode{ streamOutputIndex };
		try
		{
		...
		auto streamInputNode = la::avdecc::controller::model::StreamInputNode{ streamInputIndex };
		try
		{
		...
		else if (descriptorType == la::avdecc::entity::model::DescriptorType::StreamPortOutput)
		{
			try
			{
				...
				for (auto const& deviceMappingsKV : _listenerChannelMappings)
				{
					...
					{
						if (!controlledEntityListener->getEntity().getEntityCapabilities().test(la::avdecc::entity::EntityCapability::AemSupported) || !controlledEntityListener->hasAnyConfiguration())
						{
							continue;
						}
						...
						for (auto const& streamInput : configuration.streamInputs)
						{
							...
							if (streamInputDynamicModel.connectionInfo.state == la::avdecc::entity::model::StreamInputConnectionInfo::State::Connected)
							{
